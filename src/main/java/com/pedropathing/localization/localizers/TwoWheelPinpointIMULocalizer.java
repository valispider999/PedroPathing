...
public class TwoWheelPinpointIMULocalizer extends Localizer {
    ...
    public TwoWheelPinpointIMULocalizer(HardwareMap map, Pose setStartPose) {
        ...
        forwardEncoderPose = new Pose(0, TwoWheelPinpointIMU_ForwardY, 0);
        strafeEncoderPose = new Pose(TwoWheelPinpointIMU_StrafeX, 0, Math.toRadians(90));

        hardwareMap = map;

        pinpoint = hardwareMap.get(GoBildaPinpointDriver.class, TwoWheelPinpointIMU_Pinpoint_HardwareMapName);
        pinpoint.resetPosAndIMU();

        forwardEncoder = new Encoder(hardwareMap.get(DcMotorEx.class, TwoWheelPinpointIMU_ForwardEncoder_HardwareMapName));
        strafeEncoder = new Encoder(hardwareMap.get(DcMotorEx.class, TwoWheelPinpointIMU_StrafeEncoder_HardwareMapName));

        forwardEncoder.setDirection(TwoWheelPinpointIMU_ForwardEncoderDirection);
        strafeEncoder.setDirection(TwoWheelPinpointIMU_StrafeEncoderDirection);
